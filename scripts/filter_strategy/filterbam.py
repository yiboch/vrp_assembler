...
def filter(pen:int, out_dir):
    num_rds = []
    print("filtering...")
    for i in range(1, 3):
        inbam = pysam.AlignmentFile(out_dir + f'/r{i}/r{i}.sort.bam', 'rb')
        if not os.path.exists(out_dir):
            os.mkdir(out_dir)
        newbam_name = out_dir + f'/r{i}/readsh{i}.bam'
        ...
        while True:
            ...
            start = int(picked.pos + qalen*1/22)
            end = int(picked.pos + qalen*5/15)
            targets = int(picked.pos + qalen/20)
            targete = int(picked.reference_end + qalen/20)
            fe = int(picked.reference_end + qalen*0.05)  # only keep reads with pos after this
